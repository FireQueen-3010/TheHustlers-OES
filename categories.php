<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Categories</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <style>
        body{
            background-color:lavender;
        }
    
    </style>
</head>
<body><br><br>
        <img src="data:image/png;base64," style=" margin-left:100px;" height=240px; width=250px;   alt="html"> 
        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSE17GfPi19Erv-5QIHedCfVs0o5E0Q4PIm7g&usqp=CAU" style=" margin-left:300px;" height=250px; width=250px; alt="css">
        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQM1wCbqfyDY03K8oUpu3N-HvgMZd9eoDDDrQ&usqp=CAU"style="margin-left:300px;" height=250px; width=250px; alt="ADVANCE PROGRAMMING">
        <br><br><br>
         
        <img src="https://i.ytimg.com/vi/Qmt0QwzEmh0/maxresdefault.jpg" style="margin-left:100px; " height=260px; width=260x; alt="DATA STRUCTURES">
       <img src="https://www.wisdomjobs.com/eunivdb/iquesimg/basic-programming-interview-questions-answers.png" style="margin-left:280px;"height=260px; width=280px; alt="BASIC PROGRAMMING">


        
   
   <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
    </body>
</html>
 <!-- class="mr-3 rounded-circle img-thumbnail shadow-sm" -->